<?php
	...
	//$query = "UPDATE final.items SET item_name='$name', pic_link='$link', price = '$price', category='$category' WHERE item_id='$id'";
		
	//$query="DELETE FROM final.items WHERE item_id='$id'";
    ...
    $query2 = "SELECT item_id,item_name,pic_link,price,category FROM final.items";
	...
	if($rnr > 0){
		while($row = mysqli_fetch_assoc($result)) {
			...
			//if($count>=0&&$count<7){
				...
				echo "<div class=\"panel-footer\">".$row["price"]."<button class=\"btn btn-default\" data-toggle=\"modal\" data-target=\"#editModal\" onclick=\"javascript:openEdit('".$row["item_id"]."')\" style=\"position: absolute;bottom:25px;right:100px\">Edit</button><button class=\"btn btn-default\"style=\"position: absolute;bottom:25px;right:20px\" data-toggle=\"modal\" data-target=\"#deleteModal\" onclick=\"javascript:openDelete('".$row["item_id"]."')\">Delete</button></div>";

<?php
	...
	//$query = "SELECT item_id,item_name,pic_link,price,category FROM final.items WHERE category=$category";
		$query = "SELECT item_id,item_name,pic_link,price,category FROM final.items WHERE category=$category AND price >0 ";


	}else if($mode ==2){
		
	//$query = "SELECT item_id,item_name,pic_link,price,category FROM final.items WHERE item_name like $keyword";
		$query = "SELECT item_id,item_name,pic_link,price,category FROM final.items WHERE item_name like $keyword AND price >0 ";
	...
	if($rnr > 0){
		while($row = mysqli_fetch_assoc($result)) {
			...
			//if($count>=0&&$count<7){
				...
				//echo "<div class=\"panel-footer\">".$row["price"]."<button class=\"btn btn-default\"style=\"position: absolute;bottom:25px;right:20px\" onclick=\"javascript:addToCart('".$row["item_id"]."~~".$row["item_name"]."~~".$row["price"]."')\">Add To Cart</button></div>";
				echo "<div class=\"panel-footer\">".$row["category"]."<button class=\"btn btn-default\"style=\"position: absolute;bottom:25px;right:20px\" onclick=\"javascript:addToCart('".$row["item_id"]."~~".$row["item_name"]."~~".$row["price"]."~~".$row["category"]."')\">Add To Cart</button></div>";
				echo "</div>";
				echo "</div>";
